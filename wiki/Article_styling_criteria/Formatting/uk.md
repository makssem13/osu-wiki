...
Уся додаткова інформація про статтю, називаєма *метаданими*, або *збірним листом (front matter)*, ставляться у самий її початок. Вони пишутся у форматі [YAML](https://uk.wikipedia.org/wiki/YAML#Приклад) і відокремлюються трійними дефісами (`---`) з обох сторін, після чого іде пустий рядок.

### Статті на доробку

*Примітка: старайтеся добавляти цей тег перекладам, які треба доопрацювати. По можливості не перекладайте статті, у яких вже є цей тег.*

Якщо статтю треба переоформити, доповнити або переписати, до неї треба добавити примітку `needs_cleanup`:

```yaml
needs_cleanup: true
```

Старайтеся залишити у тілі статті [комментарі](#комментарі) з поясненням, що саме треба виправити, щоб примітку можна було прибрати. Прохання про доробку можна також оформити у вигляді issue на GitHub.

### Застаріла інформація

*Примітка: за можливості не перекладайте статті з цим тегом. Якщо тег є у оригіналу, він має бути і у перекладу.*

Якщо оригинал перекладу був оновлений або англійська стаття містить неактуальну або помилкову інформацію, до них треба додати примітку `outdated`:

```yaml
outdated: true
```

Старайтеся залишити у тілі статті [коментарі](#комментарии) з поясненням, що конкретно потребується оновити, щоб можна було прибрати цей тег.

### Застарівший переклад
...
Окрім цього, треба вказати хеш коміту, в якому була зроблена впливаюча на зміст правка, за допомогою мітки `outdated_since`:

```yaml
outdated_since: 29eac89cd535f8b071ca000af8fe4f0be22bdc9b
```

Ці теги додаються автоматично після прийняття PR із змінами, їх не треба ставити вручну. Автоматичне додавання можна [выбірково вимкнути перед прийняттям PR](/wiki/osu!_wiki/Maintenance#outdated-translations), якщо якийсь переклад в реальності не застарів.

### Ключові слова (теги)

На відміну від приміток у метаданих, ключові слова відносяться до вмісту статті та описують, по яким словам й фразам її можна нзайти через пошук по вікі. Список таких слів має бути написаним **на мові статті** та містити ключові слова оригіналу.
