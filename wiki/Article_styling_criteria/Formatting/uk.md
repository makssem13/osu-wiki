...
*Див. також: [Зміст статей](/wiki/Article_styling_criteria/Writing)*

## Локалі

*Примітка: osu! wiki показує версію статті на мові, выбраній у налаштуваннях сайта. Якщо перекладу не існує, буде показано англійську версію.*

Нижче дано список локалей, підтримуваних osu! wiki:

| Назва файла статі | Мова | Самоназва мови |
| :-- | :-- | :-- |
| `en.md` | Англійська | English |
| `ar.md` | Арабська | اَلْعَرَبِيَّةُ |
| `be.md` | Білоруська | Беларуская мова |
| `bg.md` | Болгарська | Български |
| `ca.md` | Каталанська | Català |
| `cs.md` | Чеська | Česky |
| `da.md` | Данська | Dansk |
| `de.md` | Німецька | Deutsch |
| `el.md` | Грецька | Ελληνικά |
| `es.md` | Іспанська | Español |
| `fi.md` | Фінська | Suomi |
| `fil.md` | Філіппінська | Wikang Filipino |
| `fr.md` | Французька | Français |
| `he.md` | Іврит | עִבְרִית |
| `hu.md` | Угорська | Magyar |
| `id.md` | Індонезійська | Bahasa Indonesia |
| `it.md` | Італійська | Italiano |
| `ja.md` | Японська | 日本語 |
| `ko.md` | Корейська | 한국어 |
| `lt.md` | Литовська | Lietuvių kalba |
| `nl.md` | Нідерландська | Nederlands |
| `no.md` | Норвезька | Norsk |
| `pl.md` | Польська | Polski |
| `pt.md` | Португальська | Português |
| `pt-br.md` | Бразильська португальська | Português (Brasil) |
| `ro.md` | Румунська | Română |
| `ru.md` | Російська | Русский |
| `sk.md` | Словацька | Slovenčina |
| `sl.md` | Словенська | Slovenščina |
| `sr.md` | Сербська | Српски |
| `sv.md` | Шведська | Svenska |
| `th.md` | Тайська | ไทย |
| `tr.md` | Турецька | Türkçe |
| `uk.md` | Українська | Українська мова |
| `vi.md` | В'єтнамська | Tiếng Việt |
| `zh.md` | Спрощена китайська | 简体中文 |
| `zh-tw.md` | Традиційна китайська (тайваньська) | 繁體中文（台灣） |

## Метадані

Уся додаткова інформація про статтю, яку називають *метаданими*, або *збірним листом (front matter)*, додається на самий її початок. Вона пишеться у форматі [YAML](https://uk.wikipedia.org/wiki/YAML#Приклад) і відокремлюються трьома дефісами (`---`) з обох сторін, після чого іде порожній рядок.

### Статті на доопрацювання

*Примітка: старайтеся додавати цей тег перекладам, які треба доопрацювати. По можливості не перекладайте статті, у яких вже є цей тег.*

Якщо статтю треба переоформити, доповнити або переписати, до неї треба додати примітку `needs_cleanup`:

```yaml
needs_cleanup: true
```

Старайтеся залишити у тілі статті [коментарі](#коментарі) з поясненням, що саме треба виправити, щоб примітку можна було прибрати. Прохання про доопрацювання можна також оформити у вигляді issue на GitHub.

### Застаріла інформація

*Примітка: за можливості не перекладайте статті з цим тегом. Якщо тег є у оригіналі, він має бути і у перекладу.*

Якщо оригінал перекладу був оновлений або англійська стаття містить неактуальну або помилкову інформацію, до них треба додати примітку `outdated`:

```yaml
outdated: true
```

Старайтеся залишити у тілі статті [коментарі](#коментарі) з поясненням, що конкретно потрібно оновити, щоб можна було прибрати цей тег.

### Застарілий переклад

Якщо ви змінюєте вміст англійської статті, а переклад — ні (стилістичні правки, виправлення одруків тощо — не враховується), то до перекладу треба добавити мітку `outdated_translation`:

```yaml
outdated_translation: true
```

Окрім цього, треба вказати хеш коміту, в якому була зроблена правка, що впливає на зміст, за допомогою мітки `outdated_since`:

```yaml
outdated_since: 29eac89cd535f8b071ca000af8fe4f0be22bdc9b
```

Ці теги додаються автоматично після прийняття PR із змінами, їх не треба ставити вручну. Автоматичне додавання можна [вибірково вимкнути перед прийняттям PR](/wiki/osu!_wiki/Maintenance#outdated-translations), якщо якийсь переклад в реальності не застарів.

### Ключові слова (теги)

На відміну від приміток у метаданих, ключові слова відносяться до вмісту статті та описують, за якими словами й фразами її можна знайти через пошук по вікі. Список таких слів має бути написаним **мовою статті** та містити ключові слова оригіналу.

Наприклад, стаття про систему модингу карт може мати такі ключові слова:
...
### Не перевірений переклад

*Примітка: цей тег ставлять адміністратори osu! wiki перед тим, як залити зміни.*

Буває так, що переклад статті додається в osu! wiki без перевірки носіями мови. В цьому випадку на нього ставиться примітка `no_native_review`, щоб застережити інших про можливу необхідність перевірки:

```yaml
no_native_review: true
```

Окрім того, замість мітки вище можна використовувати `no_native_review_since` для вказання коміта з самою першою неперевіреною правкою, а також користувача, який її вніс:
